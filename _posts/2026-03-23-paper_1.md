...
1. The "Back" method
    이 방법은 RNN을 시간축으로 unrolling하여 역전파 알고리즘을 그대로 적용한다. 역전파 알고리즘에서의 문제점을 그대로 갖고온다.
...
2. The "Forward" mothod
    ...
    손실에 대한 가중치의 영향은 출력이 손실에 준 영향$\left(\frac{\partial \mathcal{L}[n]}{\partial y^{(L)}_k[n]}\right)$과 가중치가 출력에 준 영향$\left(P^{L,m}_{ijk}[n]\right)$의 곱으로 표현할 수 있다.
    ...
    $$
    여기서 각 항은 다음을 의미한다.
    - $\sum_{j'} V^{(l)}_{ij'} P^{(l,m)}_{ijj'}[n-1]$: 이전 시간 단계에서의 recurrent weight를 통한 영향력 
    - $\sum_{j'} W^{(l)}_{ij'} P^{(l-1,m)}_{ijj'}[n-1]$: 이전 레이어와 이전 시간 단계에서 전달된 feed-forward 영향력
    - $\delta_{lm} y^{(l-1)}_{i}[n-1]$: kronecker delta로, $m=l$일때만 이전 레이어의 출력이 더해짐

    중요하게 볼 것은 $\sigma'(a^{(l)}_k[n])$인데 이는 
